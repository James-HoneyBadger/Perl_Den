#!/usr/bin/env bash
# ============================================================================
# bin/_badgerops_env.sh - Project environment bootstrap (bin/ copy)
#
# Sourced by the installed symlinks in ~/.local/bin or /usr/local/bin.
# Identical to the root _badgerops_env.sh but resolves BADGEROPS_ROOT_DIR relative
# to its own parent directory.
# ============================================================================
set -euo pipefail

HB_BIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BADGEROPS_ROOT_DIR="$(cd "${HB_BIN_DIR}/.." && pwd)"
