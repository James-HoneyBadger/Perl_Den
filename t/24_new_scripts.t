...
can_ok('PerlDen::Scripts::FirewallAuditor', qw(run format_report));
{
...
can_ok('PerlDen::Scripts::DockerMonitor', qw(run format_report));
{
...
can_ok('PerlDen::Scripts::BandwidthMonitor', qw(run format_report));
SKIP: {
    skip 'No /proc/net/dev (non-Linux)', 6 unless -f '/proc/net/dev';
    my $result = PerlDen::Scripts::BandwidthMonitor::run(interval => 0);
...
can_ok('PerlDen::Scripts::PackageAuditor', qw(run format_report));
{
...
can_ok('PerlDen::Scripts::SystemdAnalyzer', qw(run format_report));
{
