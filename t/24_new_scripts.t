...
# ── FirewallAuditor ──
use_ok('HBPerl::Scripts::FirewallAuditor');
can_ok('HBPerl::Scripts::FirewallAuditor', qw(run format_report));
{
    ...
    my $report = HBPerl::Scripts::FirewallAuditor::format_report($result);
    like($report, qr/FIREWALL AUDIT/, 'report contains title');
    ok(length($report) > 50, 'report has substantial content');
}

# ── DockerMonitor ──
use_ok('HBPerl::Scripts::DockerMonitor');
can_ok('HBPerl::Scripts::DockerMonitor', qw(run format_report));
{
    ...
    my $report = HBPerl::Scripts::DockerMonitor::format_report($result);
...
# ── BandwidthMonitor ──
use_ok('HBPerl::Scripts::BandwidthMonitor');
can_ok('HBPerl::Scripts::BandwidthMonitor', qw(run format_report));
SKIP: {
    skip 'No /proc/net/dev (non-Linux)', 6 unless -f '/proc/net/dev';
    my $result = HBPerl::Scripts::BandwidthMonitor::run(interval => 0);
    ...
    my $report = HBPerl::Scripts::BandwidthMonitor::format_report($result);
    like($report, qr/BANDWIDTH MONITOR/, 'report contains title');
}

# ── PackageAuditor ──
use_ok('HBPerl::Scripts::PackageAuditor');
can_ok('HBPerl::Scripts::PackageAuditor', qw(run format_report));
{
    ...
    my $report = HBPerl::Scripts::PackageAuditor::format_report($result);
    like($report, qr/PACKAGE AUDIT/, 'report contains title');
    ok(length($report) > 50, 'report has substantial content');
}

# ── SystemdAnalyzer ──
use_ok('HBPerl::Scripts::SystemdAnalyzer');
can_ok('HBPerl::Scripts::SystemdAnalyzer', qw(run format_report));
{
    my $result = HBPerl::Scripts::SystemdAnalyzer::run();
    is(ref $result, 'HASH', 'SystemdAnalyzer::run returns hashref');
    ok(exists $result->{available}, 'result has available key');
    my $report = HBPerl::Scripts::SystemdAnalyzer::format_report($result);
