#!/usr/bin/env perl
# t/23_git.t - Tests for HBPerl::Git lightweight git integration
use strict;
use warnings;
use Test::More;
...
SKIP: {
    ...
    ok(HBPerl::Git::is_git_repo($project_dir), 'project dir is a git repo');

    my $branch = HBPerl::Git::current_branch($project_dir);
    ok(defined $branch && length($branch) > 0, "current_branch returns: $branch");

    my $summary = HBPerl::Git::status_summary($project_dir);
...
ok(!HBPerl::Git::is_git_repo($tmpdir), 'temp dir is not a git repo');

my $branch = HBPerl::Git::current_branch($tmpdir);
ok(!defined $branch, 'current_branch returns undef for non-repo');

my $summary = HBPerl::Git::status_summary($tmpdir);
...
SKIP: {
    ...
    my $status = HBPerl::Git::status($project_dir);
