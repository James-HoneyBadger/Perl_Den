...
subtest 'basic echo' => sub {
    my ($out, $err, $rc) = PerlDen::Runner->run_sync('echo', 'hello world');
    chomp $out;
...
subtest 'exit code propagation' => sub {
    my ($out, $err, $rc) = PerlDen::Runner->run_sync('bash', '-c', 'exit 42');
    is($rc, 42, 'captures non-zero exit code');
};

subtest 'stderr capture' => sub {
    my ($out, $err, $rc) = PerlDen::Runner->run_sync('bash', '-c', 'echo ERR >&2');
    chomp $err;
...
subtest 'concurrent stdout and stderr (deadlock prevention)' => sub {
    ...
    my ($out, $err, $rc) = PerlDen::Runner->run_sync($cmd);
    is($rc, 0, 'exit code 0');
    my @out_lines = split /\n/, $out;
    my @err_lines = split /\n/, $err;
...
subtest 'command not found' => sub {
    my ($out, $err, $rc) = PerlDen::Runner->run_sync('nonexistent_command_xyz');
...
subtest 'single string command (backward compat)' => sub {
    my ($out, $err, $rc) = PerlDen::Runner->run_sync('echo "hello from bash"');
    chomp $out;
