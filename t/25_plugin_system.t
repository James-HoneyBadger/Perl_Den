...
local $ENV{HBPERL_HOME} = $tmpdir;

require HBPerl::ScriptRegistry;
HBPerl::ScriptRegistry->import(qw(script_index plugins_dir find_script find_closest invalidate_cache));

# ── 1. plugins_dir() returns the correct path ─────────────────────────────────
{
    my $dir = HBPerl::ScriptRegistry::plugins_dir();
    like $dir, qr/\Q$tmpdir\E/, 'plugins_dir() honours HBPERL_HOME';
}

# ── 2. No plugins — script_index still works ─────────────────────────────────
{
    HBPerl::ScriptRegistry::invalidate_cache();
    my @all = script_index();
...
# ── 4. Plugin with comment-header metadata (no metadata() sub) ────────────────
{
    ...
    print $fh <<'PM';
package HBPerl::Plugin::HeaderPlugin;
# Name: Header Plugin
...
SKIP: {
    skip 'HBPerl::Config not available', 2
        unless eval { require HBPerl::Config; 1 };

    # Write a minimal config YAML that disables GoodPlugin
    my $cfg_dir  = File::Spec->catdir($tmpdir, 'hb_perl');
