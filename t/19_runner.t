#!/usr/bin/env perl
# ============================================================================
# t/19_runner.t — Test PerlDen::Runner (synchronous mode)
# ============================================================================
use strict;
use warnings;
use Test::More;
...
subtest 'run_sync captures stdout' => sub {
    my ($stdout, $stderr, $exit) = PerlDen::Runner->run_sync('echo', 'hello world');
    chomp $stdout;
...
subtest 'run_sync captures stderr' => sub {
    my ($stdout, $stderr, $exit) = PerlDen::Runner->run_sync(
        'bash', '-c', 'echo err >&2'
    );
...
subtest 'run_sync returns exit code' => sub {
    my ($stdout, $stderr, $exit) = PerlDen::Runner->run_sync('false');
    isnt($exit, 0, 'non-zero exit code for false');
};

subtest 'run_sync single string fallback' => sub {
    my ($stdout, $stderr, $exit) = PerlDen::Runner->run_sync('echo single_string_mode');
