...
# ============================================================================
# HBPerl::ScriptRegistry v2.0 - Dynamic script discovery and plugin loader
# ============================================================================
# Built-in scripts are auto-discovered from lib/HBPerl/Scripts/*.pm — each
# module must export a metadata() function.  Plugins live in
# ~/.config/hb_perl/plugins/*.pm and must export metadata(), run(), and
# format_report().  User scripts live in ~/.config/hb_perl/scripts/*.pl
...
sub _scripts_lib_dir {
    # __FILE__ is lib/HBPerl/ScriptRegistry.pm; Scripts/ is a sibling dir
...
sub _hb_config_base {
    unless ($_hb_config_base) {
        if ($ENV{HBPERL_HOME} && -d $ENV{HBPERL_HOME}) {
            $_hb_config_base = $ENV{HBPERL_HOME};
        } else {
            eval { require File::HomeDir };
            my $home = $ENV{HOME} || (eval { File::HomeDir->my_home } // '');
            $_hb_config_base = "$home/.config/hb_perl";
        }
    }
...
sub _load_builtin_scripts {
    ...
    unless (-d $scripts_dir) {
        carp "ScriptRegistry: Scripts directory not found: $scripts_dir" if $ENV{HBPERL_DEBUG};
        $_builtin_loaded = 1;
        return ();
    }
    ...
    for my $file (@pm_files) {
        my $modname = $file;
        $modname =~ s/\.pm$//;
        my $module = "HBPerl::Scripts::$modname";
        ...
        unless ($loaded) {
            carp "ScriptRegistry: failed to load $module: $@" if $ENV{HBPERL_DEBUG};
            next;
        }

        my $meta_fn = $module->can('metadata');
        unless ($meta_fn) {
            carp "ScriptRegistry: $module has no metadata() — skipping" if $ENV{HBPERL_DEBUG};
            next;
        }

        my $meta = eval { $meta_fn->() };
        if ($@ || !ref($meta)) {
            carp "ScriptRegistry: metadata() failed for $module: $@" if $ENV{HBPERL_DEBUG};
            next;
        }
...
sub _disabled_plugins {
    ...
    my $disabled = HBPerl::Config::get('disabled_plugins') // [];
...
sub _load_plugins {
    ...
    for my $file (@pm_files) {
        my $modname = $file;
        $modname =~ s/\.pm$//;
        my $module = "HBPerl::Plugin::$modname";
        ...
        # Add the plugins dir to @INC temporarily for loading
        ...
        unless ($loaded) {
            carp "ScriptRegistry: failed to load plugin $module: $@" if $ENV{HBPERL_DEBUG};
            next;
        }
...
=head1 SYNOPSIS
    ...
    use HBPerl::ScriptRegistry qw(script_index script_categories find_script find_closest);
...
auto-discovered from C<lib/HBPerl/Scripts/*.pm> (each must export
C<metadata()>).  Plugins come from C<~/.config/hb_perl/plugins/*.pm>
and must export C<metadata()>, C<run()>, and C<format_report()>.  User
scripts come from C<~/.config/hb_perl/scripts/*.pl> and use comment
headers (C<# Name:>, C<# Description:>, C<# Category:>).

The C<HBPERL_HOME> environment variable overrides the C<~/.config/hb_perl>
base directory.

=head1 BUILT-IN SCRIPT API (metadata())

Every C<HBPerl::Scripts::*> module must export:
...
Plugins in C<~/.config/hb_perl/plugins/*.pm> must export:
...
Path to the user scripts directory (C<~/.config/hb_perl/scripts>).

=item B<plugins_dir()>

Path to the plugins directory (C<~/.config/hb_perl/plugins>).
