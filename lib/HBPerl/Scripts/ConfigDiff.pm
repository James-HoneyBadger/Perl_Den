package HBPerl::Scripts::ConfigDiff;
# ============================================================================
...
sub run {
    ...
    my $base_dir = $args{base_dir} // "$ENV{HOME}/.config/hb_perl/baselines";
...
sub _diff_against_baseline {
    ...
    for my $file (@$files) {
        ...
        if ($cur_sha eq $base_sha) {
            ...
            $entry{diff} = diff($base_file, $file, {
                STYLE      => 'Unified',
...
HBPerl::Scripts::ConfigDiff - Track changes in system configuration files
...
=head1 SYNOPSIS
    ...
    print HBPerl::Scripts::ConfigDiff::format_report($r);
