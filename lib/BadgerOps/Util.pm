package BadgerOps::Util;
# ============================================================================
# BadgerOps::Util - Shared utility functions
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub find_share_dir {
    ...
    my @candidates = (
        ...
        "$FindBin::RealBin/share",
        "/usr/share/badgerops",
        "$ENV{HOME}/.local/share/badgerops",
    );
    for my $d (@candidates) {
        return $d if -d $d;
    }
...
sub run_command {
    ...
    Carp::carp('BadgerOps::Util::run_command($string) is deprecated; use run_command_list(@cmd) instead')
        if $ENV{BADGEROPS_DEBUG} || $ENV{BADGEROPS_WARN_DEPRECATED};
    my $output = '';
...
sub slurp_file {
    ...
    open my $fh, '<:encoding(UTF-8)', $file or do {
        carp "slurp_file: cannot open $file: $!" if $ENV{BADGEROPS_DEBUG};
        return '';
    };
    local $/;
    my $content = eval { <$fh> };
...
BadgerOps::Util - Shared utility functions for the BadgerOps toolkit

=head1 SYNOPSIS

    use BadgerOps::Util qw(format_bytes run_command slurp_file is_root);
