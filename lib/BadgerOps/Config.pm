package BadgerOps::Config;
# ============================================================================
# BadgerOps::Config - Session and preference persistence
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub _ensure_dir {
    # BADGEROPS_HOME overrides the default ~/.config/badgerops base
    my $base;
    if ($ENV{BADGEROPS_HOME} && -d $ENV{BADGEROPS_HOME}) {
        $base = $ENV{BADGEROPS_HOME};
    } else {
        $base = ($ENV{HOME} || (getpwuid($<))[7] || '/tmp') . '/.config/badgerops';
    }
    $CONFIG_DIR   //= $base;
...
sub privilege_tool {
    ...
    for my $tool (qw(pkexec sudo doas)) {
        ...
        my ($path) = BadgerOps::Util::run_command_list('which', $tool);
...
BadgerOps::Config - Configuration and session management for BadgerOps IDE
...
=head1 SYNOPSIS
    ...
    my $theme = BadgerOps::Config::get('theme');
    BadgerOps::Config::set('font', 'monospace 12');
    BadgerOps::Config::save();

=head1 DESCRIPTION

Manages two YAML files under F<~/.config/badgerops/>:
...
Return the path to F<~/.config/badgerops/>.
