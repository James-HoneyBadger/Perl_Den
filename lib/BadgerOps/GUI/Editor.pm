package BadgerOps::GUI::Editor;
# ============================================================================
# BadgerOps::GUI::Editor - Tabbed source code editor with GtkSourceView
# ============================================================================
use strict;
use warnings;
use utf8;
use Glib ('TRUE', 'FALSE');
use Gtk3;
# Gtk3::SourceView is loaded via GObject Introspection in App.pm
...
sub open_file {
    ...
    BadgerOps::Config::add_recent_file($filepath);
...
sub save_file_as {
    ...
    if ($dialog->run eq 'ok') {
        ...
        $self->_save_tab($tab);
        BadgerOps::Config::add_recent_file($file);
    }
    $dialog->destroy;
}
...
sub _create_tab {
    ...
    # Apply color scheme
    my $scheme_name = BadgerOps::Config::get('editor_scheme') // 'oblivion';
    ...
    $view->set_show_line_numbers(BadgerOps::Config::get('show_line_numbers') // TRUE);
    $view->set_highlight_current_line(BadgerOps::Config::get('highlight_line') // TRUE);
    $view->set_auto_indent(BadgerOps::Config::get('auto_indent') // TRUE);
    $view->set_tab_width(BadgerOps::Config::get('tab_width') // 4);
    $view->set_insert_spaces_instead_of_tabs(TRUE);
    $view->set_smart_backspace(TRUE);
    $view->set_show_right_margin(TRUE);
    $view->set_right_margin_position(100);
    $view->set_wrap_mode(BadgerOps::Config::get('word_wrap') ? 'word' : 'none');
    $view->set_editable(!$readonly);

    # Font
    my $font = BadgerOps::Config::get('font') // 'monospace 11';
    my $font_desc = Pango::FontDescription::from_string($font);
    my $font_scale = (BadgerOps::Config::get('font_scale') // 100) / 100.0;
...
sub apply_settings {
    my ($self) = @_;
    my $font_str   = BadgerOps::Config::get('font') // 'monospace 11';
    my $font_desc  = Pango::FontDescription::from_string($font_str);
    my $font_scale = (BadgerOps::Config::get('font_scale') // 100) / 100.0;
    if ($font_scale != 1.0 && $font_desc->get_size > 0) {
        $font_desc->set_size(int($font_desc->get_size * $font_scale));
    }
    my $tab_width  = BadgerOps::Config::get('tab_width') // 4;
    my $show_ln    = BadgerOps::Config::get('show_line_numbers') // 1;
    my $hl_line    = BadgerOps::Config::get('highlight_line') // 1;
    my $wrap       = BadgerOps::Config::get('word_wrap') ? 'word' : 'none';
    my $scheme_name = BadgerOps::Config::get('editor_scheme') // 'oblivion';
    my $scheme      = $self->{scheme_manager}->get_scheme($scheme_name);
...
BadgerOps::GUI::Editor - GtkSourceView-based tabbed code editor
