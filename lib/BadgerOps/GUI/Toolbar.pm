package BadgerOps::GUI::Toolbar;
# ============================================================================
# BadgerOps::GUI::Toolbar - Menu bar and quick-access toolbar
# ============================================================================
use strict;
use warnings;
use utf8;
use Glib ('TRUE', 'FALSE');
use Gtk3;
use BadgerOps::ScriptRegistry qw(script_index);
...
sub _build_menubar {
    ...
    $self->_add_separator($edit_menu);
    $self->_add_menu_item($edit_menu, 'Preferences...',  '',           sub { BadgerOps::GUI::Dialogs::show_preferences($mw) });
    ...
    $self->_add_separator($tools_menu);
    $self->_add_menu_item($tools_menu, 'New Script from Template...', '', sub { BadgerOps::GUI::Dialogs::show_new_from_template($mw) });
    ...
    $self->_add_menu_item($run_menu, 'Run as Root',     '',    sub {
        ...
        if ($file && -f $file) {
            ...
            my $priv_tool = BadgerOps::Config::privilege_tool();
    ...
    $self->_add_menu_item($tut_menu, 'Browse Tutorials...', '', sub {
        BadgerOps::GUI::Dialogs::show_tutorial_browser($mw);
    });
    $menubar->append($self->_make_menu_heading('Tutorials', $tut_menu));

    # ── Help Menu ──
    my $help_menu = Gtk3::Menu->new;
    $self->_add_menu_item($help_menu, 'About BadgerOps IDE', '', sub { BadgerOps::GUI::Dialogs::show_about($mw) });
    $self->_add_menu_item($help_menu, 'Keyboard Shortcuts', '', sub { BadgerOps::GUI::Dialogs::show_shortcuts($mw) });
...
sub _populate_recent_menu {
    ...
    my $files = BadgerOps::Config::recent_files();
    if (!@$files) {
...
Scripts menu is dynamically populated from L<BadgerOps::ScriptRegistry>.
