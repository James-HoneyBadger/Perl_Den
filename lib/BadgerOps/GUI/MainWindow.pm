package BadgerOps::GUI::MainWindow;
# ============================================================================
# BadgerOps::GUI::MainWindow - Main application window layout
# ============================================================================
use strict;
use warnings;
use utf8;
...
use BadgerOps::Git;
use BadgerOps::Util qw(shell_quote);
use BadgerOps::GUI::Toolbar;
use BadgerOps::GUI::Editor;
use BadgerOps::GUI::Terminal;
use BadgerOps::GUI::ScriptBrowser;
use BadgerOps::GUI::Dashboard;
use BadgerOps::GUI::Dialogs;
...
sub _build_ui {
    my ($self) = @_;
    my $session = BadgerOps::Config::load_session();

    # ── Main Window ──
    my $window = Gtk3::Window->new('toplevel');
    $window->set_title('BadgerOps IDE');
    ...
    eval {
        my $icon_file = "$self->{share_dir}/icons/badgerops.png";
        if (-f $icon_file) {
    ...
    # ── Menu Bar ──
    my $toolbar = BadgerOps::GUI::Toolbar->new(main_window => $self);
    ...
    # ── Left: Script Browser ──
    my $browser = BadgerOps::GUI::ScriptBrowser->new(
        main_window => $self,
    ...
    # Dashboard tab
    my $dashboard = BadgerOps::GUI::Dashboard->new(main_window => $self);
    ...
    my $editor = BadgerOps::GUI::Editor->new(
        main_window => $self,
    ...
    my $terminal = BadgerOps::GUI::Terminal->new(
        main_window => $self,
...
sub _handle_keypress {
    ...
    if ($ctrl) {
        ...
        elsif ($key == Gtk3::Gdk::KEY_comma()) {
            # Ctrl+, → Preferences
            BadgerOps::GUI::Dialogs::show_preferences($self);
            return TRUE;
        }
        elsif ($key == Gtk3::Gdk::KEY_F()) {
            if ($shift) {
    ...
    elsif ($key == Gtk3::Gdk::KEY_F9()) {
        $self->{editor}->syntax_check;
        return TRUE;
    }
    elsif ($key == Gtk3::Gdk::KEY_F1()) {
        BadgerOps::GUI::Dialogs::show_shortcuts($self);
        return TRUE;
    }
...
sub save_state {
    ...
    BadgerOps::Config::session_set('window_width',  $w);
    BadgerOps::Config::session_set('window_height', $h);
    BadgerOps::Config::session_set('hpaned_pos', $self->{hpaned}->get_position);
    BadgerOps::Config::session_set('vpaned_pos', $self->{vpaned}->get_position);

    # Save open files
    my @files = $self->{editor}->get_open_files;
    BadgerOps::Config::session_set('open_files', \@files);

    my $current = $self->{right_notebook}->get_current_page;
    BadgerOps::Config::session_set('active_tab', $current > 0 ? $current - 1 : 0);
}
...
sub _update_git_status {
    ...
    my $summary = eval { BadgerOps::Git::status_summary($dir) };
    if (defined $summary) {
...
BadgerOps::GUI::MainWindow - Main application window and layout manager
