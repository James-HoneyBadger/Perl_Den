package BadgerOps::GUI::Terminal;
# ============================================================================
# BadgerOps::GUI::Terminal - Embedded VTE terminal and output panel
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub _build_ui {
    ...
    if ($self->{has_vte}) {
        ...
        # VTE not available — show an informational placeholder
        my $fallback_box = Gtk3::Box->new('vertical', 8);
        $fallback_box->set_valign('center');
        $fallback_box->set_halign('center');

        my $icon_label = Gtk3::Label->new;
        $icon_label->set_markup('<span size="xx-large">⌨</span>');
        $fallback_box->pack_start($icon_label, FALSE, FALSE, 0);

        my $msg_label = Gtk3::Label->new;
        $msg_label->set_markup(
            "<b>Interactive Terminal Not Available</b>\n" .
            "<span size=\"small\">Install <tt>Gtk3::Vte</tt> (package <tt>libvte-2.91-perl</tt> or <tt>perl-vte</tt>)\n" .
            "to enable the embedded terminal.\n\n" .
            "Script output is still available in the Output tab.</span>"
        );
        $msg_label->set_justify('center');
        $msg_label->set_line_wrap(TRUE);
        $fallback_box->pack_start($msg_label, FALSE, FALSE, 0);

        my $term_label = Gtk3::Label->new('⌨ Terminal');
        $notebook->append_page($fallback_box, $term_label);
    }
    ...
    my $cfg_font = BadgerOps::Config::get('font') // 'monospace 11';
...
sub _run_in_output {
    ...
    $self->{runner} = BadgerOps::Runner->new(
        on_stdout => sub {
            my ($text) = @_;
...
sub export_output {
    ...
    $dialog->set_current_name('badgerops_output.txt');
...
BadgerOps::GUI::Terminal - Embedded VTE terminal and script output panel
