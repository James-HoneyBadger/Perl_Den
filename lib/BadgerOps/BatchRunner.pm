package BadgerOps::BatchRunner;
# ============================================================================
# BadgerOps::BatchRunner - Run multiple toolkit scripts sequentially
# ============================================================================
use strict;
use warnings;
use feature 'state';
use Carp qw(croak);
use Try::Tiny;
use BadgerOps::ScriptRegistry qw(script_index find_script);
...
sub _maybe_notify {
    ...
    my $mode = BadgerOps::Config::get('notifications') // 'errors';
    ...
    my $summary = "BadgerOps batch complete";
...
sub format_batch_report {
    ...
    my $divider = '=' x 72;
    my $report  = "\n$divider\n";
    $report .= "  BADGEROPS BATCH REPORT — " . scalar(@$results) . " script(s)\n";
    $report .= "$divider\n\n";
...
<head>
<meta charset="UTF-8">
<title>BadgerOps Batch Report</title>
<style>
...
=head1 SYNOPSIS
    ...
    use BadgerOps::BatchRunner;

    my $runner = BadgerOps::BatchRunner->new(
...
Runs multiple BadgerOps toolkit scripts by name, collecting results and
producing an aggregated report. Scripts are resolved via
L<BadgerOps::ScriptRegistry/find_script>.
