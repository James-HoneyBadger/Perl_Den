package PerlDen::Runner;
# ============================================================================
# PerlDen::Runner - Script execution engine with STDOUT/STDERR capture
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub run_script {
    ...
    if ($as_root && $> != 0) {
        ...
        _write_audit_log($exec_cmd[0]);   # log before prepending privilege wrapper
    ...
    if ($pid == 0) {
        ...
        my $cmd_str = join(' ', @exec_cmd);
        print STDERR "hbperl: exec failed for '$exec_cmd[0]': $! (command: $cmd_str)\n";
        POSIX::_exit(127);
    }
...
# Write a timestamped entry to ~/.config/perlden/audit.log when a script
# is run with privilege escalation (as_root => 1).
sub _write_audit_log {
    my ($command) = @_;
    my $config_dir = PerlDen::Config::config_dir();
    my $log_file   = File::Spec->catfile($config_dir, 'audit.log');
    # Best-effort: never die if logging fails
    if (open my $fh, '>>', $log_file) {
        my $ts   = strftime('%Y-%m-%d %H:%M:%S', localtime);
        my $user = $ENV{USER} // getpwuid($<) // 'unknown';
        print $fh "[$ts] ESCALATION user=$user command=$command pid=$$\n";
        close $fh;
    }
}
...
sub run_sync {
    ...
    try {
        ...
        chomp(my $err = $_);
        return ('', "Runner: failed to spawn '$command[0]': $err", 127);
    };
    close $stdin_fh;
...
# Run a command and stream stdout/stderr directly to a file handle.
# Removes the 50K-line cap; suitable for long-running scripts with large output.
# Returns exit code.
#
# Usage: PerlDen::Runner->run_stream(\*STDOUT, @command)
#        PerlDen::Runner->run_stream($fh, @command, { timeout => 60 })
sub run_stream {
    my ($class, $out_fh, @command) = @_;
    my $opts    = ref $command[-1] eq 'HASH' ? pop @command : {};
    ...
    try {
        ...
        chomp(my $err = $_);
        print $out_fh "Runner: failed to spawn '$command[0]': $err\n";
        return 127;
    };
    close $stdin_fh;
    ...
    while ($sel->count) {
        my $wait  = $deadline > 0 ? ($deadline - time()) : undef;
        last if defined $wait && $wait <= 0;
        my @ready = $sel->can_read($wait // 1);
        last if !@ready && $deadline > 0 && time() >= $deadline;
        for my $fh (@ready) {
            my $buf;
            my $n = sysread($fh, $buf, 8192);
    ...
    if ($deadline > 0 && time() >= $deadline) {
        kill 'TERM', $pid;
        waitpid($pid, WNOHANG) or do { kill 'KILL', $pid; waitpid($pid, 0) };
        print $out_fh "Runner: command timed out after ${timeout}s\n";
        return 124;
    }
...
PerlDen::Runner - Non-blocking script execution with output capture
...
=head1 SYNOPSIS
    ...
    # Sync (CLI / tests)
    my ($stdout, $stderr, $exit) = PerlDen::Runner->run_sync('ls -la');
