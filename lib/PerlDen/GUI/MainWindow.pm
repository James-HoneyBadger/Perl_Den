package PerlDen::GUI::MainWindow;
# ============================================================================
# PerlDen::GUI::MainWindow - Main application window layout
# ============================================================================
use strict;
use warnings;
use utf8;
...
use PerlDen::GUI::Toolbar;
use PerlDen::GUI::Editor;
use PerlDen::GUI::Terminal;
use PerlDen::GUI::ScriptBrowser;
use PerlDen::GUI::Dashboard;
use PerlDen::GUI::Dialogs;
...
sub _build_ui {
    my ($self) = @_;
    my $session = PerlDen::Config::load_session();

    # ── Main Window ──
    my $window = Gtk3::Window->new('toplevel');
    $window->set_title('Perl Den IDE');
    ...
    eval {
        my $icon_file = "$self->{share_dir}/icons/perlden.png";
        if (-f $icon_file) {
    ...
    # ── Menu Bar ──
    my $toolbar = PerlDen::GUI::Toolbar->new(main_window => $self);
    ...
    # ── Left: Script Browser ──
    my $browser = PerlDen::GUI::ScriptBrowser->new(
        main_window => $self,
    ...
    # Dashboard tab
    my $dashboard = PerlDen::GUI::Dashboard->new(main_window => $self);
    ...
    my $editor = PerlDen::GUI::Editor->new(
        main_window => $self,
    ...
    my $terminal = PerlDen::GUI::Terminal->new(
        main_window => $self,
...
sub _handle_keypress {
    ...
    if ($ctrl) {
        if ($key == Gtk3::Gdk::KEY_n()) {
            ...
            # Ctrl+, → Preferences
            PerlDen::GUI::Dialogs::show_preferences($self);
            return TRUE;
        }
        elsif ($key == Gtk3::Gdk::KEY_F()) {
            if ($shift) {
        ...
        PerlDen::GUI::Dialogs::show_shortcuts($self);
        return TRUE;
    }
...
sub save_state {
    ...
    PerlDen::Config::session_set('window_width',  $w);
    PerlDen::Config::session_set('window_height', $h);
    PerlDen::Config::session_set('hpaned_pos', $self->{hpaned}->get_position);
    PerlDen::Config::session_set('vpaned_pos', $self->{vpaned}->get_position);

    # Save open files
    my @files = $self->{editor}->get_open_files;
    PerlDen::Config::session_set('open_files', \@files);

    my $current = $self->{right_notebook}->get_current_page;
    PerlDen::Config::session_set('active_tab', $current > 0 ? $current - 1 : 0);
}
...
sub _update_git_status {
    ...
    # Also update the sidebar git badge
    $self->{browser}->refresh_git_status($dir) if $self->{browser};
}
...
PerlDen::GUI::MainWindow - Main application window and layout manager
