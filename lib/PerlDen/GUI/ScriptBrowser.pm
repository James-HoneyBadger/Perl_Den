package PerlDen::GUI::ScriptBrowser;
# ============================================================================
# PerlDen::GUI::ScriptBrowser - Tree view of sysadmin script categories
# ============================================================================
use strict;
use warnings;
use utf8;
...
use PerlDen::ScriptRegistry qw(script_categories);
use PerlDen::Config;
...
sub _build_ui {
    ...
    # Git status badge — updated by refresh_git_status()
    my $git_label = Gtk3::Label->new('');
    $git_label->set_halign('start');
    $git_label->set_margin_start(12);
    $git_label->set_margin_bottom(4);
    $git_label->get_style_context->add_class('sidebar-git-status');
    $git_label->set_no_show_all(TRUE);  # hidden until a repo is detected
    $scripts_box->pack_start($git_label, FALSE, FALSE, 0);
    $self->{git_label} = $git_label;

    # Passive git refresh every 5 s (updates when browser is visible)
    my $_repo_dir = "$RealBin/..";
    Glib::Timeout->add(5000, sub {
        return FALSE unless $self->{widget} && $self->{widget}->get_mapped;
        $self->refresh_git_status($_repo_dir);
        return TRUE;
    });
    # Eager first check (deferred one frame so widget is realised)
    Glib::Idle->add(sub { $self->refresh_git_status($_repo_dir); FALSE });
...
# Update the git status badge with the current repo state.
# Called by the main window after file open/save, and by a passive timer.
sub refresh_git_status {
    my ($self, $dir) = @_;
    $dir //= "$RealBin/..";
    my $label = $self->{git_label} or return;

    my $summary = eval { PerlDen::Git::status_summary($dir) };
    if (defined $summary) {
        # Colour: green = clean, amber = dirty
        my $is_dirty = $summary !~ /\|\s*clean\s*$/;
        my $color = $is_dirty ? '#e5c07b' : '#98c379';
        my $safe  = Glib::Markup::escape_text($summary);
        $label->set_markup(
            qq{<span font='Adwaita Sans 9' foreground='$color'>\x{e0a0} $safe</span>}
        );
        $label->show;
    } else {
        $label->set_text('');
...
sub _show_context_menu {
    ...
    $run_root->signal_connect(activate => sub {
        my $priv_tool = PerlDen::Config::privilege_tool();
...
PerlDen::GUI::ScriptBrowser - Sidebar tree view of sysadmin scripts

=head1 DESCRIPTION

Displays the 15 toolkit scripts in a categorised tree view populated
from L<PerlDen::ScriptRegistry>.  Double-click opens a script in the
