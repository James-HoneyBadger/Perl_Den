package PerlDen::GUI::Editor;
# ============================================================================
# PerlDen::GUI::Editor - Tabbed source code editor with GtkSourceView
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub save_file_as {
    ...
    if ($dialog->run eq 'ok') {
        ...
        $self->_save_tab($tab);
        PerlDen::Config::add_recent_file($file);
    }
    $dialog->destroy;
}
...
sub _create_tab {
    ...
    # Apply color scheme
    my $scheme_name = PerlDen::Config::get('editor_scheme') // 'oblivion';
    ...
    $view->set_show_line_numbers(PerlDen::Config::get('show_line_numbers') // TRUE);
    $view->set_highlight_current_line(PerlDen::Config::get('highlight_line') // TRUE);
    $view->set_auto_indent(PerlDen::Config::get('auto_indent') // TRUE);
    $view->set_tab_width(PerlDen::Config::get('tab_width') // 4);
    $view->set_insert_spaces_instead_of_tabs(TRUE);
    $view->set_smart_backspace(TRUE);
    $view->set_show_right_margin(TRUE);
    $view->set_right_margin_position(100);
    $view->set_wrap_mode(PerlDen::Config::get('word_wrap') ? 'word' : 'none');
    $view->set_editable(!$readonly);

    # Font
    my $font = PerlDen::Config::get('font') // 'monospace 11';
    my $font_desc = Pango::FontDescription::from_string($font);
    my $font_scale = (PerlDen::Config::get('font_scale') // 100) / 100.0;
...
sub apply_settings {
    my ($self) = @_;
    my $font_str   = PerlDen::Config::get('font') // 'monospace 11';
    my $font_desc  = Pango::FontDescription::from_string($font_str);
    my $font_scale = (PerlDen::Config::get('font_scale') // 100) / 100.0;
    if ($font_scale != 1.0 && $font_desc->get_size > 0) {
        $font_desc->set_size(int($font_desc->get_size * $font_scale));
    }
    my $tab_width  = PerlDen::Config::get('tab_width') // 4;
    my $show_ln    = PerlDen::Config::get('show_line_numbers') // 1;
    my $hl_line    = PerlDen::Config::get('highlight_line') // 1;
    my $wrap       = PerlDen::Config::get('word_wrap') ? 'word' : 'none';
    my $scheme_name = PerlDen::Config::get('editor_scheme') // 'oblivion';
    my $scheme      = $self->{scheme_manager}->get_scheme($scheme_name);
...
PerlDen::GUI::Editor - GtkSourceView-based tabbed code editor
