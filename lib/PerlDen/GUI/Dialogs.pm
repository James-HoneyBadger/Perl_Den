package PerlDen::GUI::Dialogs;
# ============================================================================
# PerlDen::GUI::Dialogs - About, Preferences, Templates, Tutorials dialogs
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub show_about {
    ...
    $dialog->set_program_name('Perl Den IDE');
    $dialog->set_version($PerlDen::App::VERSION // '1.00');
    ...
    $dialog->set_website('https://github.com/James-HoneyBadger/Perl Den');
...
sub show_preferences {
    ...
    my $font_btn = Gtk3::FontButton->new_with_font(PerlDen::Config::get('font') // 'monospace 11');
    ...
    $tab_spin->set_value(PerlDen::Config::get('tab_width') // 4);
    ...
    $scale_slider->set_value(PerlDen::Config::get('font_scale') // 100);
    $scale_slider->set_hexpand(TRUE);
    $scale_slider->set_size_request(200, -1);
    my $scale_value_label = Gtk3::Label->new(sprintf('%d%%', PerlDen::Config::get('font_scale') // 100));
    ...
    my $current_theme = PerlDen::Config::get('theme') // 'vscode-dark-plus';
    my $theme_idx = 0;
    for my $i (0 .. $#themes) {
    ...
    my $current_scheme = PerlDen::Config::get('editor_scheme') // 'oblivion';
    my $active_idx = 0;
    for my $i (0 .. $#schemes) {
    ...
    $dash_spin->set_value(PerlDen::Config::get('dashboard_refresh_seconds') // 5);
    ...
    my $cur_notify = PerlDen::Config::get('notifications') // 'errors';
    my $notify_idx = 1;
    ...
    my $plugin_desc = Gtk3::Label->new(
        "Drop .pm files into ~/.config/perlden/plugins/ to install plugins.\n" .
    ...
    my $pdir         = plugins_dir();
    my @disabled_now = @{ PerlDen::Config::get('disabled_plugins') // [] };
    ...
    if ($dialog->run eq 'ok') {
        ...
        PerlDen::Config::set('font',              $font_btn->get_font_name);
        PerlDen::Config::set('tab_width',         $tab_spin->get_value_as_int);
        PerlDen::Config::set('font_scale',        int($scale_slider->get_value));
        my $sel_idx = $theme_combo->get_active;
        my $theme_id = ($sel_idx >= 0 && $sel_idx <= $#themes)
            ? $themes[$sel_idx][1]
            : 'vscode-dark-plus';
        PerlDen::Config::set('theme',             $theme_id);
        PerlDen::Config::set('editor_scheme',     $scheme_combo->get_active_text);
        my $ni = $notify_combo->get_active;
        PerlDen::Config::set('notifications',
            ($ni >= 0 && $ni <= $#notify_values) ? $notify_values[$ni][1] : 'errors');
        PerlDen::Config::set('dashboard_refresh_seconds', $dash_spin->get_value_as_int);
        PerlDen::Config::set('show_line_numbers', $ln_check->get_active ? 1 : 0);
        PerlDen::Config::set('highlight_line',    $hl_check->get_active ? 1 : 0);
        PerlDen::Config::set('word_wrap',         $wrap_check->get_active ? 1 : 0);
        ...
        PerlDen::Config::set('disabled_plugins', \@new_disabled);
        invalidate_cache();
        PerlDen::Config::save();
...
sub show_tutorial_browser {
    ...
    $text_buffer->create_tag('body', foreground => $tc->{fg}, font => PerlDen::Config::get('font') // 'monospace 11');
...
PerlDen::GUI::Dialogs - Modal dialogs for the Perl Den IDE
