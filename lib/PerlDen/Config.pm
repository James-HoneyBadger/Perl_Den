package PerlDen::Config;
# ============================================================================
# PerlDen::Config - Session and preference persistence
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub _ensure_dir {
    ...
    if ($ENV{PERLDEN_HOME} && -d $ENV{PERLDEN_HOME}) {
        $base = $ENV{PERLDEN_HOME};
    } else {
        $base = ($ENV{HOME} || (getpwuid($<))[7] || '/tmp') . '/.config/perlden';
    }
    $CONFIG_DIR   //= $base;
...
sub privilege_tool {
    ...
    for my $tool (qw(pkexec sudo doas)) {
        ...
        my ($path) = PerlDen::Util::run_command_list('which', $tool);
...
PerlDen::Config - Configuration and session management for PerlDen IDE
...
=head1 SYNOPSIS
    ...
    PerlDen::Config::load();
    my $theme = PerlDen::Config::get('theme');
    PerlDen::Config::set('font', 'monospace 12');
    PerlDen::Config::save();

=head1 DESCRIPTION

Manages two YAML files under F<~/.config/perlden/>:
...
Return the path to F<~/.config/perlden/>.
