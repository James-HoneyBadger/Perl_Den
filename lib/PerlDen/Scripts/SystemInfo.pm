package PerlDen::Scripts::SystemInfo;
# ============================================================================
...
sub run {
    ...
    if (open my $fh, '<', '/proc/meminfo') {
        ...
        # Guard against negative values if /proc/meminfo parsing fails
        my $_mu = $info{mem_total_kb} - $info{mem_available_kb};
        $info{mem_used_kb}      = $_mu > 0 ? $_mu : 0;
        $info{swap_total_kb}    = $mem{SwapTotal} // 0;
        $info{swap_free_kb}     = $mem{SwapFree}  // 0;
        my $_su = $info{swap_total_kb} - $info{swap_free_kb};
        $info{swap_used_kb}     = $_su > 0 ? $_su : 0;
    }
...
PerlDen::Scripts::SystemInfo - Comprehensive Linux system information collector

=head1 SYNOPSIS

    use PerlDen::Scripts::SystemInfo;
    my $info = PerlDen::Scripts::SystemInfo::run();
    print PerlDen::Scripts::SystemInfo::format_report($info);
