package PerlDen::Util;
# ============================================================================
# PerlDen::Util - Shared utility functions
# ============================================================================
use strict;
use warnings;
use utf8;
...
sub run_command {
    ...
    Carp::carp('PerlDen::Util::run_command($string) is deprecated; use run_command_list(@cmd) instead')
        if $ENV{PERLDEN_DEBUG} || $ENV{PERLDEN_WARN_DEPRECATED};
    my $output = '';
...
sub slurp_file {
    ...
    open my $fh, '<:encoding(UTF-8)', $file or do {
        carp "slurp_file: cannot open $file: $!" if $ENV{PERLDEN_DEBUG};
        return '';
    };
    local $/;
    my $content = eval { <$fh> };
...
PerlDen::Util - Shared utility functions for the PerlDen toolkit

=head1 SYNOPSIS

    use PerlDen::Util qw(format_bytes run_command slurp_file is_root);
