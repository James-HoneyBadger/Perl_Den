#!/usr/bin/env bash
# ============================================================================
# _badgerops_env.sh - Project environment bootstrap (root copy)
#
# Sourced by the top-level launcher scripts (badgerops, badgerops-cli, badgerops-tui,
# badgerops-gui).  Sets PATH, PERL5LIB, and share-directory variables, and
# provides helper functions for dependency checking.
# ============================================================================
set -euo pipefail

BADGEROPS_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

export BADGEROPS_ROOT_DIR
export BADGEROPS_SHARE_DIR="${BADGEROPS_ROOT_DIR}/share"
export BADGEROPS_SCRIPTS_DIR="${BADGEROPS_ROOT_DIR}/scripts"

# Preload project paths
export PATH="${BADGEROPS_ROOT_DIR}/bin:${PATH}"
if [[ -n "${PERL5LIB:-}" ]]; then
  export PERL5LIB="${BADGEROPS_ROOT_DIR}/lib:${PERL5LIB}"
else
  export PERL5LIB="${BADGEROPS_ROOT_DIR}/lib"
fi

# Ensure config dir exists
mkdir -p "${HOME}/.config/badgerops"
...
check_perl_module() {
  local module="$1"
  perl -I"${BADGEROPS_ROOT_DIR}/lib" -M"${module}" -e 1 >/dev/null 2>&1
}
...
check_gui_deps() {
  ...
  if (( ${#missing[@]} > 0 )); then
    ...
    echo "       Install with: cpanm --installdeps \"${BADGEROPS_ROOT_DIR}\"" >&2
    return 1
  fi
}
...
preflight_gui() {
  ...
  if perl -I"${BADGEROPS_ROOT_DIR}/lib" -e '
  ...
  # 9. Theme CSS exists
  local theme_dir="${BADGEROPS_ROOT_DIR}/share/themes"
  ...
  for subdir in icons templates tutorials themes; do
    if [[ -d "${BADGEROPS_ROOT_DIR}/share/${subdir}" ]]; then
  ...
  if (( errors > 0 )); then
    ...
    if (( ${#gui_missing[@]} > 0 )); then
      echo "       Install Perl deps: cpanm --installdeps \"${BADGEROPS_ROOT_DIR}\"" >&2
    fi
    return 1
  else
    _bo_ok "All checks passed — launching BadgerOps IDE"
    echo "" >&2
    return 0
  fi
}
